import React, { useState } from 'react';
import { Prism as SyntaxHighlighter } from 'react-syntax-highlighter';
import { vscDarkPlus } from 'react-syntax-highlighter/dist/esm/styles/prism';
import { Copy } from 'lucide-react';

const FONTS = {
  COMIC_SANS: '"Comic Sans MS", cursive',
  PAPYRUS: 'Papyrus, fantasy',
  TIMES_NEW_ROMAN: '"Times New Roman", serif',
  IMPACT: 'Impact, sans-serif',
  BRUSH_SCRIPT: '"Brush Script MT", cursive',
  WINGDINGS: 'Wingdings, sans-serif'
};

const COLORS = {
  YELLOW: '#FFFF00',
  CYAN: '#00FFFF',
  MAGENTA: '#FF00FF',
  LIME: '#00FF00',
  RED: '#FF0000',
  BLUE: '#0000FF'
};

const inputConfigs = {
  input1: {
    name: 'Shaky Rainbow Input',
    className: 'px-8 py-6 bg-gradient-to-r from-red-500 via-yellow-500 to-purple-500 text-black font-black text-2xl border-8 border-black rounded-lg shake shadow-2xl placeholder-black',
    placeholder: 'Type something AMAZING!',
    reactCode: `<input
  type="text"
  className="px-8 py-6 bg-gradient-to-r from-red-500 via-yellow-500 to-purple-500 text-black font-black text-2xl border-8 border-black rounded-lg shake shadow-2xl placeholder-black"
  placeholder="Type something AMAZING!"
  style={{ fontFamily: "Comic Sans MS" }}
/>`,
    htmlCode: `<input
  type="text"
  class="px-8 py-6 bg-gradient-to-r from-red-500 via-yellow-500 to-purple-500 text-black font-black text-2xl border-8 border-black rounded-lg shake shadow-2xl placeholder-black"
  placeholder="Type something AMAZING!"
  style="font-family: 'Comic Sans MS';"
/>`,
    cssCode: `.input-shaky-rainbow {
  padding: 1.5rem 2rem;
  background: linear-gradient(to right, #ef4444, #eab308, #a855f7);
  color: #000000;
  font-weight: 900;
  font-size: 1.5rem;
  border: 8px solid #000000;
  border-radius: 0.5rem;
  font-family: 'Comic Sans MS';
  animation: shake 0.5s infinite;
  box-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.9);
  outline: none;
}

.input-shaky-rainbow::placeholder {
  color: #000000;
  opacity: 0.8;
}

.input-shaky-rainbow:focus {
  background: linear-gradient(to right, #dc2626, #ca8a04, #7c3aed);
  animation: shake 0.3s infinite;
}

@keyframes shake {
  0%, 100% { transform: translateX(0) rotate(0deg); }
  25% { transform: translateX(-8px) rotate(-2deg); }
  75% { transform: translateX(8px) rotate(2deg); }
}`
  },

  input2: {
    name: 'Blinking Neon Input',
    className: 'px-10 py-7 bg-black text-cyan-400 font-black text-3xl border-12 border-cyan-400 rounded-full blink shadow-lg placeholder-cyan-400',
    placeholder: 'ENTER NEON TEXT HERE!',
    reactCode: `<input
  type="text"
  className="px-10 py-7 bg-black text-cyan-400 font-black text-3xl border-12 border-cyan-400 rounded-full blink shadow-lg placeholder-cyan-400"
  placeholder="ENTER NEON TEXT HERE!"
  style={{ fontFamily: "Impact", textShadow: "0 0 10px #00FFFF" }}
/>`,
    htmlCode: `<input
  type="text"
  class="px-10 py-7 bg-black text-cyan-400 font-black text-3xl border-12 border-cyan-400 rounded-full blink shadow-lg placeholder-cyan-400"
  placeholder="ENTER NEON TEXT HERE!"
  style="font-family: Impact; text-shadow: 0 0 10px #00FFFF;"
/>`,
    cssCode: `.input-blinking-neon {
  padding: 1.75rem 2.5rem;
  background-color: #000000;
  color: #00ffff;
  font-weight: 900;
  font-size: 1.875rem;
  border: 12px solid #00ffff;
  border-radius: 9999px;
  font-family: Impact;
  text-shadow: 0 0 10px #00ffff;
  animation: blink 0.7s infinite;
  box-shadow: 0 25px 50px -12px rgba(0, 255, 255, 0.5);
  outline: none;
}

.input-blinking-neon::placeholder {
  color: #00ffff;
  text-shadow: 0 0 5px #00ffff;
  opacity: 0.8;
}

.input-blinking-neon:focus {
  animation: blink 0.3s infinite;
  box-shadow: 0 0 30px 15px rgba(0, 255, 255, 0.7);
}

@keyframes blink {
  0%, 50% { 
    opacity: 1;
    border-color: #00ffff;
  }
  51%, 100% { 
    opacity: 0.5;
    border-color: #ff00ff;
  }
}`
  },

  input3: {
    name: 'Spinning 3D Input',
    className: 'px-12 py-8 bg-lime-400 text-red-600 font-black text-4xl border-16 border-red-600 rounded-xl spin-3d placeholder-red-600',
    placeholder: 'SPINNING INPUT!',
    reactCode: `<input
  type="text"
  className="px-12 py-8 bg-lime-400 text-red-600 font-black text-4xl border-16 border-red-600 rounded-xl spin-3d placeholder-red-600"
  placeholder="SPINNING INPUT!"
  style={{ fontFamily: "Papyrus", transformStyle: "preserve-3d" }}
/>`,
    htmlCode: `<input
  type="text"
  class="px-12 py-8 bg-lime-400 text-red-600 font-black text-4xl border-16 border-red-600 rounded-xl spin-3d placeholder-red-600"
  placeholder="SPINNING INPUT!"
  style="font-family: Papyrus; transform-style: preserve-3d;"
/>`,
    cssCode: `.input-spinning-3d {
  padding: 2rem 3rem;
  background-color: #84cc16;
  color: #dc2626;
  font-weight: 900;
  font-size: 2.25rem;
  border: 16px solid #dc2626;
  border-radius: 0.75rem;
  font-family: Papyrus;
  animation: spin3d 3s infinite linear;
  transform-style: preserve-3d;
  box-shadow: 0 35px 60px -12px rgba(0, 0, 0, 0.6);
  outline: none;
  perspective: 1000px;
}

.input-spinning-3d::placeholder {
  color: #dc2626;
  opacity: 0.8;
}

.input-spinning-3d:focus {
  animation: spin3d 1s infinite linear;
  background-color: #65a30d;
}

@keyframes spin3d {
  0% { transform: rotateY(0deg) rotateX(5deg); }
  100% { transform: rotateY(360deg) rotateX(5deg); }
}`
  },

  input4: {
    name: 'Bouncing Comic Input',
    className: 'px-14 py-9 bg-purple-600 text-yellow-300 font-black text-5xl border-20 border-yellow-300 rounded-full bounce-comic placeholder-yellow-300',
    placeholder: 'BOUNCE WHILE TYPING!',
    reactCode: `<input
  type="text"
  className="px-14 py-9 bg-purple-600 text-yellow-300 font-black text-5xl border-20 border-yellow-300 rounded-full bounce-comic placeholder-yellow-300"
  placeholder="BOUNCE WHILE TYPING!"
  style={{ fontFamily: "Comic Sans MS", textShadow: "3px 3px 0 #000" }}
/>`,
    htmlCode: `<input
  type="text"
  class="px-14 py-9 bg-purple-600 text-yellow-300 font-black text-5xl border-20 border-yellow-300 rounded-full bounce-comic placeholder-yellow-300"
  placeholder="BOUNCE WHILE TYPING!"
  style="font-family: 'Comic Sans MS'; text-shadow: 3px 3px 0 #000;"
/>`,
    cssCode: `.input-bouncing-comic {
  padding: 2.25rem 3.5rem;
  background-color: #9333ea;
  color: #fef08a;
  font-weight: 900;
  font-size: 3rem;
  border: 20px solid #fef08a;
  border-radius: 9999px;
  font-family: 'Comic Sans MS';
  animation: bounceComic 1s infinite alternate;
  text-shadow: 3px 3px 0 #000000;
  box-shadow: 0 45px 70px -15px rgba(0, 0, 0, 0.7);
  outline: none;
}

.input-bouncing-comic::placeholder {
  color: #fef08a;
  text-shadow: 2px 2px 0 #000000;
  opacity: 0.8;
}

.input-bouncing-comic:focus {
  animation: bounceComic 0.5s infinite alternate;
  background-color: #7c3aed;
}

@keyframes bounceComic {
  0% { 
    transform: translateY(0) rotate(-3deg);
  }
  100% { 
    transform: translateY(-20px) rotate(3deg);
  }
}`
  },

  input5: {
    name: 'Pulsing Glow Input',
    className: 'px-16 py-10 bg-blue-500 text-white font-black text-6xl border-24 border-white pulse-glow placeholder-white',
    placeholder: 'GLOWING TEXT INPUT!',
    reactCode: `<input
  type="text"
  className="px-16 py-10 bg-blue-500 text-white font-black text-6xl border-24 border-white pulse-glow placeholder-white"
  placeholder="GLOWING TEXT INPUT!"
  style={{ fontFamily: "Brush Script MT" }}
/>`,
    htmlCode: `<input
  type="text"
  class="px-16 py-10 bg-blue-500 text-white font-black text-6xl border-24 border-white pulse-glow placeholder-white"
  placeholder="GLOWING TEXT INPUT!"
  style="font-family: 'Brush Script MT';"
/>`,
    cssCode: `.input-pulsing-glow {
  padding: 2.5rem 4rem;
  background-color: #3b82f6;
  color: #ffffff;
  font-weight: 900;
  font-size: 3.75rem;
  border: 24px solid #ffffff;
  font-family: 'Brush Script MT';
  animation: pulseGlow 2s infinite alternate;
  box-shadow: 0 0 40px 20px rgba(59, 130, 246, 0.8);
  outline: none;
}

.input-pulsing-glow::placeholder {
  color: #ffffff;
  opacity: 0.9;
}

.input-pulsing-glow:focus {
  animation: pulseGlow 0.5s infinite alternate;
  box-shadow: 0 0 60px 30px rgba(59, 130, 246, 1);
}

@keyframes pulseGlow {
  0% { 
    transform: scale(1);
    box-shadow: 0 0 40px 20px rgba(59, 130, 246, 0.8);
  }
  100% { 
    transform: scale(1.05);
    box-shadow: 0 0 60px 30px rgba(59, 130, 246, 1);
  }
}`
  },

  input6: {
    name: 'Matrix Color Flip Input',
    className: 'px-18 py-11 bg-black text-green-400 font-mono font-black text-6xl border-28 border-green-400 matrix-flip placeholder-green-400',
    placeholder: 'MATRIX MODE ACTIVATED!',
    reactCode: `<input
  type="text"
  className="px-18 py-11 bg-black text-green-400 font-mono font-black text-6xl border-28 border-green-400 matrix-flip placeholder-green-400"
  placeholder="MATRIX MODE ACTIVATED!"
  style={{ textShadow: "0 0 8px #00FF00" }}
/>`,
    htmlCode: `<input
  type="text"
  class="px-18 py-11 bg-black text-green-400 font-mono font-black text-6xl border-28 border-green-400 matrix-flip placeholder-green-400"
  placeholder="MATRIX MODE ACTIVATED!"
  style="text-shadow: 0 0 8px #00FF00;"
/>`,
    cssCode: `.input-matrix-flip {
  padding: 2.75rem 4.5rem;
  background-color: #000000;
  color: #00ff00;
  font-family: monospace;
  font-weight: 900;
  font-size: 3.75rem;
  border: 28px solid #00ff00;
  text-shadow: 0 0 8px #00ff00;
  animation: matrixFlip 0.6s infinite;
  box-shadow: 0 0 35px 15px rgba(0, 255, 0, 0.6);
  outline: none;
  letter-spacing: 1px;
}

.input-matrix-flip::placeholder {
  color: #00ff00;
  text-shadow: 0 0 5px #00ff00;
  opacity: 0.8;
}

.input-matrix-flip:focus {
  animation: matrixFlip 0.2s infinite;
  box-shadow: 0 0 50px 25px rgba(0, 255, 0, 0.8);
}

@keyframes matrixFlip {
  0%, 100% { 
    color: #00ff00;
    border-color: #00ff00;
    background-color: #000000;
  }
  33% { 
    color: #ff00ff;
    border-color: #ff00ff;
    background-color: #ffff00;
  }
  66% { 
    color: #00ffff;
    border-color: #00ffff;
    background-color: #ff00ff;
  }
}`
  }
};

export function InputSample() {
  const [selectedInput, setSelectedInput] = useState('input1');
  const [viewMode, setViewMode] = useState('preview');
  const [codeLanguage, setCodeLanguage] = useState('react');
  const [styleType, setStyleType] = useState('tailwindcss');
  const [copiedCode, setCopiedCode] = useState(false);
  const [copiedCss, setCopiedCss] = useState(false);
  const [inputValue, setInputValue] = useState('');

  const config = inputConfigs[selectedInput];

  const handleCopy = (text, type) => {
    navigator.clipboard.writeText(text);
    if (type === 'code') {
      setCopiedCode(true);
      setTimeout(() => setCopiedCode(false), 2000);
    } else {
      setCopiedCss(true);
      setTimeout(() => setCopiedCss(false), 2000);
    }
  };

  const getCodeToDisplay = () => {
    if (styleType === 'tailwindcss') {
      return codeLanguage === 'react' ? config.reactCode : config.htmlCode;
    } else {
      return codeLanguage === 'react' 
        ? `<input\n  type="text"\n  className="input-custom"\n  placeholder="${config.placeholder}"\n/>`
        : `<input\n  type="text"\n  class="input-custom"\n  placeholder="${config.placeholder}"\n/>`;
    }
  };

  const renderInputPreview = () => {
    const inputProps = {
      type: "text",
      className: config.className,
      placeholder: config.placeholder,
      value: inputValue,
      onChange: (e) => setInputValue(e.target.value),
      style: { 
        fontFamily: config.name.includes('Matrix') ? 'monospace' : 
                  config.name.includes('Comic') ? FONTS.COMIC_SANS :
                  config.name.includes('Brush') ? FONTS.BRUSH_SCRIPT :
                  config.name.includes('Neon') ? FONTS.IMPACT :
                  config.name.includes('3D') ? FONTS.PAPYRUS : FONTS.COMIC_SANS,
        textShadow: config.name.includes('Neon') ? '0 0 10px #00FFFF' :
                   config.name.includes('Comic') ? '3px 3px 0 #000' :
                   config.name.includes('Matrix') ? '0 0 8px #00FF00' : 'none'
      }
    };

    return <input {...inputProps} />;
  };

  return (
    <div className="min-h-screen p-4 md:p-8" style={{ 
      background: 'linear-gradient(135deg, #FF00FF 0%, #00FFFF 50%, #FFFF00 100%)',
      fontFamily: FONTS.COMIC_SANS
    }}>
      <style>{`
        @keyframes shake {
          0%, 100% { transform: translateX(0) rotate(0deg); }
          25% { transform: translateX(-8px) rotate(-2deg); }
          75% { transform: translateX(8px) rotate(2deg); }
        }
        @keyframes blink {
          0%, 50% { 
            opacity: 1;
            border-color: #00ffff;
          }
          51%, 100% { 
            opacity: 0.5;
            border-color: #ff00ff;
          }
        }
        @keyframes spin3d {
          0% { transform: rotateY(0deg) rotateX(5deg); }
          100% { transform: rotateY(360deg) rotateX(5deg); }
        }
        @keyframes bounceComic {
          0% { 
            transform: translateY(0) rotate(-3deg);
          }
          100% { 
            transform: translateY(-20px) rotate(3deg);
          }
        }
        @keyframes pulseGlow {
          0% { 
            transform: scale(1);
            box-shadow: 0 0 40px 20px rgba(59, 130, 246, 0.8);
          }
          100% { 
            transform: scale(1.05);
            box-shadow: 0 0 60px 30px rgba(59, 130, 246, 1);
          }
        }
        @keyframes matrixFlip {
          0%, 100% { 
            color: #00ff00;
            border-color: #00ff00;
            background-color: #000000;
          }
          33% { 
            color: #ff00ff;
            border-color: #ff00ff;
            background-color: #ffff00;
          }
          66% { 
            color: #00ffff;
            border-color: #00ffff;
            background-color: #ff00ff;
          }
        }
        @keyframes colorflip {
          0%, 100% { color: #FFFF00; }
          33% { color: #FF00FF; }
          66% { color: #00FFFF; }
        }
        .shake { animation: shake 0.5s infinite; }
        .blink { animation: blink 0.7s infinite; }
        .spin-3d { animation: spin3d 3s infinite linear; }
        .bounce-comic { animation: bounceComic 1s infinite alternate; }
        .pulse-glow { animation: pulseGlow 2s infinite alternate; }
        .matrix-flip { animation: matrixFlip 0.6s infinite; }
        .colorflip { animation: colorflip 2s infinite; }
      `}</style>

      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8 bg-black/20 p-6 border-8 border-lime-400 transform -rotate-2">
          <h1
            className="text-4xl md:text-6xl font-black mb-4 colorflip"
            style={{ fontFamily: FONTS.COMIC_SANS }}
          >
            🎨 DUMBUI INPUTS 🎨
          </h1>
          <p
            className="text-xl md:text-2xl font-bold"
            style={{ fontFamily: FONTS.TIMES_NEW_ROMAN, color: COLORS.CYAN }}
          >
            The WORST Input Fields on the Internet!
          </p>
        </div>

        <div className="mb-6 bg-gradient-to-r from-pink-500 via-yellow-400 to-cyan-500 p-4 border-8 border-red-600 transform rotate-1">
          <div className="flex flex-wrap gap-3 justify-center">
            {Object.keys(inputConfigs).map((input) => (
              <button
                key={input}
                onClick={() => setSelectedInput(input)}
                className={`px-6 py-3 font-black text-lg border-4 transition-all ${
                  selectedInput === input
                    ? 'bg-lime-400 text-black border-black scale-110 shake'
                    : 'bg-purple-600 text-yellow-300 border-yellow-300 hover:scale-105'
                }`}
                style={{ fontFamily: FONTS.PAPYRUS }}
              >
                {inputConfigs[input].name}
              </button>
            ))}
          </div>
        </div>

        <div className="mb-6 bg-cyan-400 p-4 border-8 border-magenta-600 transform -rotate-1">
          <div className="flex flex-wrap gap-4 justify-center items-center">
            <button
              onClick={() => setViewMode('preview')}
              className={`px-6 py-3 font-black text-lg border-4 transition-all ${
                viewMode === 'preview'
                  ? 'bg-yellow-300 text-black border-black shake'
                  : 'bg-blue-600 text-white border-white hover:bg-blue-700'
              }`}
              style={{ fontFamily: FONTS.COMIC_SANS }}
            >
              👁️ PREVIEW
            </button>
            <button
              onClick={() => setViewMode('code')}
              className={`px-6 py-3 font-black text-lg border-4 transition-all ${
                viewMode === 'code'
                  ? 'bg-yellow-300 text-black border-black shake'
                  : 'bg-blue-600 text-white border-white hover:bg-blue-700'
              }`}
              style={{ fontFamily: FONTS.COMIC_SANS }}
            >
              💻 CODE
            </button>

            <div className="flex gap-2">
              <select
                value={styleType}
                onChange={(e) => setStyleType(e.target.value)}
                className="px-4 py-3 font-black text-lg border-4 border-black bg-lime-400"
                style={{ fontFamily: FONTS.PAPYRUS }}
              >
                <option value="tailwindcss">TailwindCSS</option>
                <option value="css">CSS</option>
              </select>

              <select
                value={codeLanguage}
                onChange={(e) => setCodeLanguage(e.target.value)}
                className="px-4 py-3 font-black text-lg border-4 border-black bg-pink-400"
                style={{ fontFamily: FONTS.PAPYRUS }}
              >
                <option value="react">React</option>
                <option value="html">HTML</option>
              </select>
            </div>
          </div>
        </div>

        <div className="bg-purple-600 p-8 border-8 border-yellow-300 transform rotate-1">
          {viewMode === 'preview' ? (
            <div className="flex flex-col items-center justify-center min-h-[400px] bg-gradient-to-br from-red-500 via-green-500 to-blue-500 border-8 border-cyan-400 p-8">
              <h2
                className="text-3xl font-black mb-8 colorflip"
                style={{ fontFamily: FONTS.PAPYRUS }}
              >
                {config.name}
              </h2>
              <div className="w-full max-w-2xl">
                {renderInputPreview()}
                <div className="mt-4 text-center">
                  <p className="text-white font-bold text-lg" style={{ fontFamily: FONTS.COMIC_SANS }}>
                    You typed: <span className="text-yellow-300">{inputValue || '(nothing yet)'}</span>
                  </p>
                </div>
              </div>
            </div>
          ) : (
            <div className="space-y-4">
              <div className="bg-gray-900 p-4 border-4 border-lime-400 relative">
                <div className="flex justify-between items-center mb-2 pb-2 border-b-2 border-gray-700">
                  <span className="text-yellow-300 font-bold" style={{ fontFamily: FONTS.COMIC_SANS }}>
                    {codeLanguage === 'react' ? '⚛️ REACT CODE' : '🌐 HTML CODE'}
                  </span>
                  <button
                    onClick={() => handleCopy(getCodeToDisplay(), 'code')}
                    className="px-3 py-1 bg-cyan-400 text-black font-bold border-2 border-black hover:bg-cyan-300 flex items-center gap-2"
                    style={{ fontFamily: FONTS.COMIC_SANS }}
                  >
                    <Copy size={16} />
                    {copiedCode ? 'COPIED!' : 'COPY'}
                  </button>
                </div>
                <SyntaxHighlighter
                  language={codeLanguage === 'react' ? 'jsx' : 'html'}
                  style={vscDarkPlus}
                  customStyle={{
                    margin: 0,
                    borderRadius: 0,
                    fontSize: '14px',
                    maxHeight: '400px'
                  }}
                  showLineNumbers
                  wrapLines
                >
                  {getCodeToDisplay()}
                </SyntaxHighlighter>
              </div>

              {styleType === 'css' && (
                <div className="bg-gray-900 p-4 border-4 border-pink-400 relative">
                  <div className="flex justify-between items-center mb-2 pb-2 border-b-2 border-gray-700">
                    <span className="text-pink-300 font-bold" style={{ fontFamily: FONTS.COMIC_SANS }}>
                      🎨 CSS CODE
                    </span>
                    <button
                      onClick={() => handleCopy(config.cssCode, 'css')}
                      className="px-3 py-1 bg-pink-400 text-black font-bold border-2 border-black hover:bg-pink-300 flex items-center gap-2"
                      style={{ fontFamily: FONTS.COMIC_SANS }}
                    >
                      <Copy size={16} />
                      {copiedCss ? 'COPIED!' : 'COPY'}
                    </button>
                  </div>
                  <SyntaxHighlighter
                    language="css"
                    style={vscDarkPlus}
                    customStyle={{
                      margin: 0,
                      borderRadius: 0,
                      fontSize: '14px',
                      maxHeight: '400px'
                    }}
                    showLineNumbers
                    wrapLines
                  >
                    {config.cssCode}
                  </SyntaxHighlighter>
                </div>
              )}
            </div>
          )}
        </div>

        <div className="mt-8 bg-gradient-to-r from-yellow-400 via-pink-500 to-cyan-400 p-6 border-8 border-green-500 transform -rotate-2">
          <p
            className="text-2xl font-black text-center"
            style={{ fontFamily: FONTS.PAPYRUS, color: '#000' }}
          >
            ⚠️ WARNING: Typing in these inputs may cause SEIZURES! ⚠️
          </p>
        </div>
      </div>
    </div>
  );
}